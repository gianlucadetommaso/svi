...
class SVIModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.save_hyperparameters(
            ignore=["model", "svi_algorithm", "target_log_prob_fn"]
        )
        ...
        self.param_shapes = {
            n: p.shape for n, p in self.base_model.named_parameters() if p.requires_grad
        }
        self.param_numels = {
            n: p.numel()
        ...
        self.particles = nn.Parameter(
            initial_params.unsqueeze(0).repeat(num_particles, 1), requires_grad=True
        )
    ...
    def _freeze_all_except_last_layer(self, model: nn.Module):
        ...
        modules_with_params = [
            m for m in model.modules() if len(list(m.parameters(recurse=False))) > 0
        ]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self._target_log_prob_fn is not None:
            ...
            def compute_loss_single_particle(p_vec, x_batch, y_batch):
                ...
                y_hat = torch.func.functional_call(
                    self.base_model, current_state, (x_batch,)
                )
        ...
        # --- Common Update Step ---
        direction = self._svi_algorithm.compute_direction(
            self.particles.detach(), score_stack
        )
    ...
    def _compute_loss(self, y_hat, y):
        if isinstance(y, torch.Tensor) and (
            y.dtype == torch.long or y.dtype == torch.int
        ):
